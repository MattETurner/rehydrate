...
impl SubBlock {
    /// Validate that we've consumed at most `size` bytes; tolerate
    /// trailing unread bytes (newer firmware adds fields the parser
    /// doesn't model). Skips the unread tail so the next read picks up
    /// at the right place.
    pub fn validate_size(
        ...
        if end_offset > expected_offset {
            return Err(ParseError::invalid(format!(
                "Subblock over-read: got {end_offset:x} expected {expected_offset:x}"
            )));
        }
        if end_offset < expected_offset {
