...
//!     cargo test -p rmsync-sync --test full_pull_smoke -- --ignored --nocapture
//! ```
//!
...
use rmsync_sync::{execute_pull, plan_pull, progress, ProgressEvent};
...
async fn full_pull_against_real_device() {
    ...
    println!(
        ...
        plan.items
            .iter()
            .filter(|p| matches!(p.status, rmsync_sync::PlanItemStatus::New))
            .count(),
        plan.items
            .iter()
            .filter(|p| matches!(p.status, rmsync_sync::PlanItemStatus::Changed))
            .count(),
        plan.items
            .iter()
            .filter(|p| matches!(p.status, rmsync_sync::PlanItemStatus::Unchanged))
            .count(),
        plan.items
            .iter()
            .filter(|p| matches!(p.status, rmsync_sync::PlanItemStatus::Skipped))
            .count(),
    );
